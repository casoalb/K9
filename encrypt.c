...
#ifdef ENCRYPT_MD5
    snprintf(dest, size, "%s", crypt(src, "$1$tmpsalt$"));
    return 0;
#endif
...
#endif
	return 1;
#ifdef BROKEN_MD5_COMPAT
    {
	int i;
	memset(buf+16, 0, 16);
	for (i = 0; i < 32; i += 2)
	    buf[i/2] = XTOI(buf[i])<<4 | XTOI(buf[i+1]);
    }
    if (memcmp(buf, password, 16) == 0)
	return 1;
#endif
...
void genpass(char *pass) {
  char *usable = "abcdefghijklmnopqrstuvwxyz"
                 "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
                 "1234567890";
  uint16 i;

  for( i = 0; i < 8; i++)
    pass[i] = usable[(int)(61.0 * rand() / (RAND_MAX + 1.0))];
